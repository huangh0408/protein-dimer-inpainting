...
loss_D = loss_adv_D*beta_local+loss_adv_D_global*beta_global # * lambda_adv
...
#    labels_D_hh = tf.concat( [tf.random_uniform([batch_size],minval=0.799+s,maxval=1.0,dtype=tf.float32), tf.random_uniform([batch_size],minval=0,maxval=0.201-s,dtype=tf.float32)] ,0)
   ...
   # elif epoch % 20 == 0: 
    #    fake_length=5
     #   true_length=batch_size-fake_length
      #  labels_D = tf.concat( (np.random.random(fake_length)*ssss,np.random.random(true_length)*ssss+sss ,np.random.random(fake_length)*ssss+sss ,np.random.random(true_length)*ssss) ,0) 
   # else:
    #    ss=(epoch/(5*n_epochs+0.1))
     #   sss=0.8+ss
      #  ssss=0.2-ss
