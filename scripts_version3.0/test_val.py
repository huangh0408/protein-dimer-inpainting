...
test_chain_ori=list(map(lambda x: load_chain( x,net_size ), test_chain_paths))
test_images_ori = list(map(lambda x: load_image(x,net_size), test_image_paths))

#    test_images_crop = map(lambda x: crop_random(x, x=32, y=32), test_images_ori)
test_images_crop = list(map(lambda x,y: crop_interaction(x, y,net_size), test_images_ori,test_chain_ori))
